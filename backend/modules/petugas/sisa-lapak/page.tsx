...
export default function SisaLapakPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const list: KecamatanData[] = Array.isArray(res) ? res : [];
      setData(list);
      setLoadError(null);
      // Pertahankan tab kecamatan yang lagi aktif kalau masih ada di data
      // baru -- cuma jatuh ke kecamatan pertama kalau belum ada yang aktif
      // sama sekali (mis. saat halaman pertama kali dimuat) atau kalau
      // kecamatan yang tadi aktif sudah nggak ada lagi di data.
      setActiveKecamatan((prev) => {
        if (prev && list.some((k) => k.kecamatan === prev)) return prev;
        return list.length > 0 ? list[0].kecamatan : null;
      });
    } catch (err) {
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-sm">
                    ...
                    <div className="flex flex-wrap items-center gap-sm">
                      ...
                      <button
                        ...
                        className="pt-btn pt-btn-ghost-danger"
                      >

...
// Placeholder dashboard buat role PETUGAS. Sudah nyambung ke
// GET /api/petugas/dashboard (dilindungi AuthMiddleware + RoleMiddleware
// "petugas" di backend) -- kontennya masih dummy, tinggal diganti begitu
// halaman verifikasi pengajuan beneran mulai dikerjakan.
export default function PetugasDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/petugas/dashboard`, {
    ...
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-md rounded-lg border border-outline-variant bg-surface-container-lowest p-3xl text-center">
        ...
        <p className="text-body-md text-on-surface-variant">
          Akun kamu tidak memiliki akses ke halaman Petugas CFD.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-lg">
      <div>
        <h2 className="text-headline-lg text-on-surface">Dashboard Petugas</h2>
        <p className="mt-xs max-w-2xl text-body-md text-on-surface-variant">
          Ringkasan pengajuan usaha yang perlu diverifikasi.
        </p>
      </div>
          ...
          <div>
            ...
            <p className="mt-xs text-body-md text-on-surface-variant">
              {state === "loading"
                ...
                : state === "error"
                  ? message
                  : message || "Berhasil terhubung ke backend sebagai petugas."}
            </p>
          </div>
        </div>
      </section>
